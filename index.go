...
func main() {
	...
	input, _ := ioutil.ReadFile("sample.txt") // read the input file

	// (FUNC 1) find "(hex)" then convert the previous element from 16 to 32
	firstSoS := strings.Split(string(input), " ") // split the input into a slice of strings (file casted from byte to string)
